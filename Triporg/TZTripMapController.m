...
@interface TZTripMapController ()
{
    ...
    MKCoordinateRegion pastLocation;
...
- (void)viewDidLoad
{
    ...
    if (!_trip.mapCache) {
        ...
        self->pastLocation = self.mapView.region;
    ...
    //self->pastLocation = nil;
    //NSLog(@"past location = %d", self->pastLocation.location.coordinate.latitude);
}
...
- (void)showUserHeadding:(id)sender
{
    ...
    else
    {
        ...
        //self.mapView.userTrackingMode = MKUserTrackingModeNone;
        _navigate = !_navigate;
        [self.mapView setCenterCoordinate:self->pastLocation.center animated:YES];
        [UIView transitionWithView:locateButton duration:0.5
