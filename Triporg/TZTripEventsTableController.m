...
@interface TZTripEventsTableController () {
    NSString *versioniOS;
    UIView *topView;
    FXLabel *titleLabel;
    FXLabel *dateLabel;
...
@implementation TZTripEventsTableController
...
- (void)viewDidLoad
{
    ...
    else
    {
        ...
        saveNavigationView = [[UIView alloc] initWithFrame:CGRectMake(0, -70, self.view.bounds.size.width, 70)];
...
- (void)editPressed:(id)sender
{
    ...
    if (_editing) {
        ...
        if (_allEvents)
        {
            ...
            searchBar.layer.borderWidth = 0.3f;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    NSArray *events = [_eventsByDays objectAtIndex:section - 1];
    
    TZTripEvent *tripEvent = [events objectAtIndex:0];
    ...
    header.showMapCallback = ^(TZTripEventHeader *header_) {
        ...
        TZTripMapController *mapController = [storyboard instantiateViewControllerWithIdentifier:@"Map"];
        mapController.title = NSLocalizedString(@"Mapa", @"");
        
        if (!_trip.mapCache)
        {
            mapController.tripEvents = events;
            mapController.cityId = _trip.city_id;
        }
        else
        {
            mapController.trip = _trip;
        }
